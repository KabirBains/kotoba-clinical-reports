...
// Minimal valid .docx structure - no external template needed
...
<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:style w:type="paragraph" w:styleId="Heading1"><w:name w:val="heading 1"/><w:pPr><w:keepNext/></w:pPr><w:rPr><w:b/><w:sz w:val="28"/></w:rPr></w:style>
  <w:style w:type="paragraph" w:styleId="Heading2"><w:name w:val="heading 2"/><w:pPr><w:keepNext/></w:pPr><w:rPr><w:b/><w:sz w:val="24"/></w:rPr></w:style>
  <w:style w:type="paragraph" w:styleId="Heading3"><w:name w:val="heading 3"/><w:pPr><w:keepNext/></w:pPr><w:rPr><w:b/><w:sz w:val="22"/></w:rPr></w:style>
</w:styles>`;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!input.participant?.fullName) {
      return new Response(JSON.stringify({ error: "participant.fullName is required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
...
<w:body>${newBody}${sectPr}</w:body>
    ...
    let binary = "";
    for (let i = 0; i < docxBytes.length; i++) { binary += String.fromCharCode(docxBytes[i]); }
    ...
    return new Response(JSON.stringify({ success: true, file: base64, fileName }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e: any) {
